...
def _(Nt, cases, np, os, samples):
    ...
    filepath = os.path.join('../data', f"benchmark_target_Nt_{formatted_Nt}.npy")

    # Check if data is saved and load it, otherwise generate and save
    if os.path.isfile(filepath):
        X = np.load(filepath)
        print(f"Loaded data for benchmark target")
    else:
        qs = cases.target(Nt)
        X = np.array([q[-samples:] for q in qs])
        np.save(filepath, X)
        print(f"Generated and saved data for benchmark target")
...
def _(
    ...
    _fig, _ax_main, _, _ = surd.plot_states(data=_data[:, :, _pos], bins=bins_1, target=_target, source=_source, save_path=os.path.join('../figures', 'benchmark_target_R12_states.pdf'), title='$\\rm{Redundant}$  $\\rm{causality}$', xlabel='$q_{1}$ \\rm{(source)}', ylabel='$q_{2}^+$ \\rm{(target)}', vmax=np.max(_data), vmin=-np.max(_data), cmap=_custom_cmap, norm=False)
    return
...
def _(
    ...
    _custom_cmap = LinearSegmentedColormap.from_list('custom_cmap', _colors, N=256)
    _fig, _ax_main, _, _ = surd.plot_states(data=_data, bins=bins_1, target=_target, source=_source, save_path=os.path.join('../figures', f'benchmark_target_U{_source}_states.pdf'), title='$\\rm{Unique}$  $\\rm{causality}$', xlabel='$q_{1}$ \\rm{(source)}', ylabel='$q_{2}^+$ \\rm{(target)}', vmax=np.max(_data), vmin=-np.max(_data), cmap=_custom_cmap, norm=False)
    return
...
def _(
    ...
    _custom_cmap = LinearSegmentedColormap.from_list('custom_cmap', _colors, N=256)
    _fig, _ax_main, _, _ = surd.plot_states(data=_data[:, :, _pos], bins=bins_1, target=_target, source=_source, save_path=os.path.join('../figures', f'benchmark_target_S12_states.pdf'), title='$\\rm{Synergistic}$  $\\rm{causality}$', xlabel='$q_{1}$ \\rm{(source)}', ylabel='$q_{2}^+$ \\rm{(target)}', vmax=np.max(_data), vmin=-np.max(_data), cmap=_custom_cmap, norm=False)
    return
